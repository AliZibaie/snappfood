...
@section('authentication-form')
    ...
    <section class="min-h-screen flex items-stretch text-white ">
        <div class="lg:flex w-1/2 hidden bg-gray-500 bg-no-repeat bg-cover relative items-center" style="background-image: url({{url('assets/images/login.jpg')}});">
            <div class="absolute bg-black opacity-60 inset-0 z-0"></div>
            <div class="w-full px-24 z-10">
                <h1 class="text-5xl font-bold text-left tracking-wide">شما خاص هستید</h1>
                <p class="text-3xl my-4">پس یه غذا از اسنپ فود بزنید </p>
            </div>
        ...
        <div class="lg:w-1/2 w-full flex items-center justify-center text-center md:px-16 px-0 z-0" style="background-color: #161616;">
            <div class="w-full py-6 z-20">
                ...
                <form action="{{route('login')}}" class="sm:w-2/3 w-full px-4 lg:px-0 mx-auto" method="post">
                    ...
                    <div class="pb-2 pt-4 ">
                        <label for="password"></label><input class="block w-full p-4 text-lg rounded-sm   text-right" style="background-color: #282828" type="password" name="password" id="password" placeholder="پسورد">
                        @error('password')
                        <p class="text-red-800 text-xl">{{$message}}</p>
                        @enderror
                    </div>
                    <div class="text-right text-gray-400 hover:underline hover:text-gray-100">
                        <a href="{{route('show.register')}}">میخواهید ثبت نام کنید؟اینجا کلیک کنید</a>
                    </div>
                    <div class="px-4 pb-2 pt-4">
                        <button class="uppercase block w-full p-4 text-lg rounded-full bg-indigo-500 hover:bg-indigo-600 focus:outline-none" type="submit">ورود</button>
                    </div>
